...
function writeJsonAtomic(filePath, value) {
  ...
  try {
    ...
    try {
      ...
      // Best effort cleanup van een niet-publieke tijdelijke bestandsnaam.
    }
  }
}
...
function parseBody(req) {
  return new Promise((resolve, reject) => {
    ...
    req.on('end', () => {
      ...
      settled = true;
      if (tooLarge) {
        return reject(Object.assign(new Error('Payload te groot'), { code: 'BODY_TOO_LARGE' }));
      }
...
function serializeCookie(name, value, { httpOnly = true, secure = false } = {}) {
...
function setLocalSessionCookies(req, res, token) {
  ...
  appendSetCookie(res, serializeCookie(SESSION_COOKIE, token, { httpOnly: true, secure }));
  appendSetCookie(res, serializeCookie(SESSION_HINT_COOKIE, '1', { httpOnly: false, secure }));
}
...
async function withPasswordWork(res, work) {
  if (activePasswordWork >= MAX_CONCURRENT_PASSWORD_WORK) {
    ...
    sendJson(res, 429, {
      message: 'De beveiligde inlogcontrole is even druk. Probeer het over een paar seconden opnieuw.',
    });
...
function currentRateStatus(req) {
  const clientKey = getClientRateKey(req);
  return { clientKey, status: failureLimiter.status(clientKey) };
}

async function rejectNonAdminPasswordAttempt(req, res, password) {
  const { clientKey, status } = currentRateStatus(req);
  if (!status.allowed) return sendRateLimit(res, status);
  const work = await withPasswordWork(res, () => burnPasswordAttempt(password || ''));
  if (work.handled) return undefined;
...
async function authenticateAdmin(req, res, { name = '', username = '', password = '' } = {}) {
  ...
  if (!currentAdmin) {
    return sendJson(res, 409, {
      message: 'Het account is ondertussen gewijzigd. Probeer opnieuw in te loggen.',
    });
  }
...
async function changeAdminPassword(req, res) {
  ...
  if (!freshAdmin) {
    return sendJson(res, 409, {
      message: 'Het account is ondertussen gewijzigd. Log opnieuw in.',
    });
  }
  ...
  // Andere sessies behouden bewust hun oude fingerprint. De buitenste
  // securitylaag trekt die bij het eerstvolgende request in. Alleen de actieve
  // sessie waarmee dit wachtwoord is gewijzigd wordt naar de nieuwe fingerprint gezet.
  ...
  if (!currentSession) {
    return sendJson(res, 409, {
      message: 'Je sessie is gewijzigd. Log opnieuw in met je nieuwe wachtwoord.',
    });
  }
...
async function handleLocalPasswordRoute(req, res, requestUrl) {
  if (req.method === 'POST' && requestUrl.pathname === '/api/login-by-name') {
    ...
    if (body.type !== 'staff') {
      return rejectNonAdminPasswordAttempt(req, res, body.password);
    }
    return authenticateAdmin(req, res, { name: body.name, password: body.password });
  }
  ...
  if (req.method === 'POST' && requestUrl.pathname === '/api/login') {
    ...
    return authenticateAdmin(req, res, { username: body.username, password: body.password });
  }
...
function wrapRequestListener(listener) {
  return async function localPasswordAuthListener(req, res) {
    let requestUrl;
    try {
    ...
    try {
      ...
      console.error('[Local Password Auth] Interne fout:', error?.message || error);
