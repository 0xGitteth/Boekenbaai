...
- `eigen boek`, `vast in de klas` and `Klassenboek` are semantic markers only in recognized context columns such as `naam leerling`, `Geleend door klas`, `Klassen` and `Aanwezig bieb`. Bibliographic fields are never interpreted as context markers: a book whose literal title is `Eigen boek` remains an ordinary title. In context, `eigen boek` excludes that row from the school collection, while `vast in de klas` and `Klassenboek` mean a school-library book with a fixed classroom location that still needs review.
